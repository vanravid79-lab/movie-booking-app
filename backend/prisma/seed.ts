...
async function main() {
  ...
  const c1 = await prisma.cinema.create({
    data: {
      ...
      cinema_location:
        "Samdach Sothearos Blvd, Sangkat Tonle Bassac, Phnom Penh",
  ...
  const c3 = await prisma.cinema.create({
    data: {
      ...
      cinema_location:
        "Phnom Penh City Center, Sangkat Srah Chak, Khan Daun Penh",
  ...
  const c4 = await prisma.cinema.create({
    data: {
      ...
      cinema_location:
        "Corner of St. 315 & St. 516, Sangkat Boeung Kak 1, Toul Kork",
  ...
  const c5 = await prisma.cinema.create({
    data: {
      ...
      halls: {
        create: [
          {
            hall_name: "Gold Class",
            hall_type: "VIP Recliner",
            hall_capacity: 60,
          },
  ...
  const c6 = await prisma.cinema.create({
    data: {
      ...
      cinema_location:
        "St. 271, Sangkat Tomnoub Teuk, Khan Chamkarmon, Phnom Penh",
  ...
  fallbackList.forEach((d) => {
    if (!movieMap.has(d.id)) {
      movieMap.set(d.id, {
        id: d.id,
        title: d.title,
        overview: "Now showing in cinemas.",
      });
    }
  });
  ...
  console.log(
    `✅ Finished: ${allCinemas.length} Cinemas | ${targetMovies.length} Movies | ${schedulesData.length} Showtimes`,
  );
}
